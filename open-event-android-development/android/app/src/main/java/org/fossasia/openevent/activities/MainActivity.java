...
public class MainActivity extends BaseActivity implements FeedAdapter.AdapterCallback {
    ...
    private void setNavHeader(Event event) {
        ...
        if (!Utils.isEmpty(logo)) {
            ...
            OpenEventApp.picassoWithCache.load(R.drawable.pageheader).into(headerView);  // .mipmap.ic_launcher //Changed Header ICON
        }
    }
                            ...
                            (dialogInterface, button) -> {
                                switch (button) {
                                    case DialogInterface.BUTTON_POSITIVE:
                                        //startDownloadFromNetwork();                     //Commented by saheed 2-Oct-2017, since data get loads from assets
                                        downloadFromAssets();
                                        break;
